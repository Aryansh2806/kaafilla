...
- On save (onboarding in `authStore.completeOnboarding`, edits in `EditProfile`), if a face photo + declared gender are present, `detectGender()` (`src/verification/index.ts`) POSTs the photo to the gender service and the result is written to `profiles.gender_check` via `saveGenderCheck()`: `'match'` if it agrees with the declaration, else `'needs_review'`. A null/failed detection is **non-blocking** (stays `unchecked`). `detected_gender` stores the model's guess for a human reviewer only.
...
- **The lock is relaxed while under review** (stage14): the owner *may* change `lead` while `gender_check = 'needs_review'` so an honest mistake self-corrects (re-running the check on save); once it settles (`match`/`appealed`) it locks again. A service-role (Studio) change always bypasses the trigger — that's how a reviewer approves/rejects an appeal (`update profiles set gender_check = 'match' | 'needs_review'`). There is **no admin UI**; reviewing is manual SQL for now.

The service is `services/gender-detect/` — a Flask + `opencv-python-headless` app (Levi-Hassner Adience Caffe model) deployed as a Docker web service on **Render**, auth'd by an `X-Api-Key` header. The app reaches it via `EXPO_PUBLIC_GENDER_API_URL` + `EXPO_PUBLIC_GENDER_API_KEY` in `.env` (git-ignored; free tier cold-starts ~30–60s). **This is client-trusted and self-forgeable today** (a user could POST their own `match`) — production should move `detectGender` + `saveGenderCheck` behind an Edge Function, exactly like the `markVerified` note. Face gender-classification is biased/unreliable, which is *why* the model only ever flags-for-review and never auto-rejects.
