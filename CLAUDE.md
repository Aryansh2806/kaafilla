...
## Gender declaration & photo check (spans app + a Python service + 3 migrations)

Gender = **`profiles.lead`** (`'women' | 'men'`) — the same field that drives every gendered feature (ratios, WOMEN-LED, host badges). There is **no ML gender *detection* of record**: the user *declares* Woman/Man (`ProfileBuild`, required), and it's **locked once set** by the `lock_profile_lead()` trigger (stage12). A separate **soft, advisory** photo check runs on top and never blocks:

- On save (onboarding in `authStore.completeOnboarding`, edits in `EditProfile`), if a face photo + declared gender are present, the client invokes `runGenderCheck()` (`src/api/auth.ts`), which calls the **`gender-check` Edge Function**. The function reads the caller's `lead` + first photo *from the DB* (client can't lie about either), calls the gender model with a **server-held key**, and writes `profiles.gender_check` with the **service role**: `'match'` if it agrees with the declaration, else `'needs_review'`. A null/failed/no-face detection is **non-blocking** (state unchanged). `detected_gender` stores the model's guess for a human reviewer only.
- `gender_check` states (stage13/14): `unchecked | match | needs_review | appealed`. `MyProfile` shows a banner for `needs_review` ("Fix in Edit" or "Appeal — my choice is right" → `appealGenderCheck()` → `appealed`) and an "Appeal submitted" banner for `appealed`.
- **`gender_check`/`detected_gender` are server-authoritative** (stage15): the `guard_gender_check()` trigger forbids the owner from writing them, so a client can't forge `'match'`. The *one* owner-allowed change is the appeal (`needs_review → appealed`). Service role (the Edge Function, or a Studio/reviewer action — `auth.uid() IS NULL`) bypasses it.
- **The `lead` lock is relaxed while under review** (stage14): the owner *may* change `lead` while `gender_check = 'needs_review'` so an honest mistake self-corrects (re-running the check on save); once it settles (`match`/`appealed`) it locks again. A reviewer approves/rejects an appeal via service-role SQL (`update profiles set gender_check = 'match' | 'needs_review'`). There is **no admin UI**; reviewing is manual SQL for now.

The model service is `services/gender-detect/` — a Flask + `opencv-python-headless` app (Levi-Hassner Adience Caffe model) deployed as a Docker web service on **Render**, auth'd by an `X-Api-Key` header. Only the `gender-check` Edge Function talks to it, via the `GENDER_API_URL` + `GENDER_API_KEY` **Supabase secrets** (not `EXPO_PUBLIC_*`, so nothing ships in the app bundle; free tier cold-starts ~30–60s). Deploy per `supabase/GENDER_CHECK_SETUP.md`; if it's not deployed the check simply no-ops (`unchecked`). Face gender-classification is biased/unreliable, which is *why* the model only ever flags-for-review and never auto-rejects.
